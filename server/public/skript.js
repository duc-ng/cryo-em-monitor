...
*/
  function init(firstDataPoint) {
    ...
    for (
      i = fixedDataPoints;
      i < Object.keys(firstDataPoint).length;
      i = i + 2
    ) {
      //create div
...
*/
  function updateActivity(datapoint) {
    ...
    if (datapoint._mmsdateProcessed_Value != default_value) {
      var value = parseInt(
        document.getElementById("imagesProcessed").innerHTML
      );
    ...
    if (datapoint._mmsdateExported_Value != default_value) {
      var value = parseInt(
        document.getElementById("processingErrors").innerHTML
      );
  ...
  //main: receive new images
  socket.on("newImages", function (image) {
